        ...
        <div class="dashboard-box">
            ...
            <main class="main-content">
                ...
                <div class="totals flex flex-row gap-2">
                    <p>
                        Total Classes: <span>{{ classesCount }}</span>
                    </p>
                    <p class="total-holiday">
                        Holidays: <span>{{ holidayCount }}</span>
                    </p>
                    <p class="total-no-class">
                        No Classes: <span>{{ noClassCount }}</span>
                    </p>
                    <p class="total-absent">
                        Absents: <span>{{ absentCount }}</span>
                    </p>
                    <p class="total-excused">
                        Total Excused: <span>{{ excusedCount }}</span>
                    </p>
                </div>
                    ...
                    <table id="student-table">
                        ...
                        <tbody>
                            <tr v-for="(student, index) in studentsArray" :key="student.id">
                                ...
                                <td>
                                    {{ attendanceData[student.student_number]?.present || 0 }}
                                </td>
                                <td>
                                    {{ attendanceData[student.student_number]?.absent || 0 }}
                                </td>
                                <td>
                                    {{ attendanceData[student.student_number]?.excused || 0 }}
                                </td>
                                <td>
                                    <a :href="'/attendance-list/' + student.student_number">Edit</a>
                                </td>
                            </tr>
                        </tbody>
                    </table>
...
const fetchData = async () => {
    try {
        ...
        studentsArray.value = students.sort((a, b) =>
...
const fetchAttendance = async (class_id) => {
    try {
        ...
        noClassCount.value += attendanceArray.filter(
            (record) => record.status === "no-class"
        ).length;
        holidayCount.value += attendanceArray.filter(
            (record) => record.status === "holiday"
        ).length;
        presentCount.value += attendanceArray.filter(
            (record) => record.status === "present"
        ).length;
        absentCount.value += attendanceArray.filter(
            (record) => record.status === "absent"
        ).length;
        excusedCount.value += attendanceArray.filter(
